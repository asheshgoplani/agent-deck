...
- **BREAKING: `agent-deck add -t <title>` exits 1 instead of 0 when the session already exists.** Registering a duplicate printed a success banner and exited 0 while writing nothing, so a script could not tell a new session from one that already existed, and a caller that went on to `session start` the title was operating on a session it had not created. The duplicate is now reported as an error (`--json` emits `"code": "ALREADY_EXISTS"`), and `add --help` documents both the exit code and the without-`-t` behavior, which is unchanged: the session is registered under a numbered title with a warning on stderr and exit 0. A session's identity is its title plus where it runs, so two sessions sharing a title on different hosts, or in different directories, are not duplicates. Note that commands taking a title still resolve to the first exact match, so prefer session IDs when you deliberately reuse a title across locations. If you have a script that treats `add` as idempotent, tolerate exit 1 (`add … || true`) or check with `list` first.
