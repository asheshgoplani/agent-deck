...
- **`agent-deck web --no-tui` flag for headless CI server starts.** `web` always set up the bubbletea TUI alongside the HTTP listener, even when invoked from a non-interactive environment. On Linux CI runners (no controlling TTY) `tea.NewProgram`'s cancel-reader fails its epoll registration with `error creating cancelreader: bubbletea: error creating cancel reader: add reader to epoll interest list`, the process exits, and the HTTP server is torn down before any test client can connect — exactly the failure that disabled `lighthouse-ci.yml` and broke the Lighthouse step of `weekly-regression.yml` from 2026-04-10 onward (per the v1.7.42 audit, `.github/workflows/README.md`). The new `--no-tui` flag on `agent-deck web` keeps the HTTP server running in its existing goroutine and replaces the bubbletea init with a `select {}` block that the existing global SIGINT/SIGTERM handler at `cmd/agent-deck/main.go:434` cleans up. The flag returns from `buildWebServer` as a second bool so the gate lives in `main.go` (not on the `web.Server` struct, which has no other TUI awareness). Verified with `TestBuildWebServer_NoTUIFlag` (parsing parity for `--no-tui`, `-no-tui`, `--no-tui=true`, `--no-tui=false`, default-off) and a smoke run that confirms `/healthz` returns 200 in a sandbox where the regular `web` invocation reproduces the exact epoll error from the audit notes. Out of scope: any other subcommand. The TUI subcommand path is unchanged.
...
- **Lighthouse CI gates on bundle delta vs base ref, not just absolute thresholds.** The v1.7.42 audit's failure mode — absolute thresholds calibrated against the v1.7.30 bundle, never bumped, every subsequent PR red — is structurally avoided going forward. `.github/workflows/lighthouse-ci.yml` now drops `treosh/lighthouse-ci-action@v12` (which only enforces absolute thresholds) and runs `lhci collect` twice per PR: once on the PR head, once on the checked-out base ref. After the existing `lhci assert` against `.lighthouserc.json` (still the coarse upper bound), `tests/lighthouse/compare-deltas.mjs` reads both result dirs, takes the median per metric, and fails the gate if `total-byte-weight` or `resource-summary:script:size` grew more than `MAX_BYTE_WEIGHT_DELTA_PCT` / `MAX_SCRIPT_SIZE_DELTA_PCT` (default 5% each, env-overrideable in the workflow). Timing metrics (FCP / LCP / TBT / Speed Index) stay on absolute warn-only because shared-runner timing variance overwhelms a 5% delta. Bootstrap: this PR is the one that adds `--no-tui`, so the base ref's binary cannot start headlessly; the base collect step is `continue-on-error: true` and the script exits 0 with a logged warning when no base lhr-*.json files are present. Every PR after this one merges runs through the full delta gate. Lighthouse results from both collects are uploaded as a workflow artifact (`lighthouse-results-<run_id>`, 14-day retention) for post-mortem inspection.

- **Maintainer override on the Lighthouse delta gate via the `lighthouse-regression-acknowledged` label.** A delta-gate failure was a hard merge block in the first cut of `lighthouse-ci.yml`, which is the wrong default for a soft signal: most legit bundle bumps happen at the natural cadence of feature work, not in dedicated "calibration" PRs, and a hard block would push reviewers to disable the gate rather than acknowledge the regression (literally the v1.7.42 dynamic). New flow: the delta gate's compare step runs with `continue-on-error: true`; a follow-up override step checks for the `lighthouse-regression-acknowledged` label on `github.event.pull_request.labels` and exits 0 (with a `::notice::` annotation naming the override) if present, else exits 1 (with an `::error::` annotation explaining how to opt in). The workflow's `pull_request` trigger gains the `labeled` and `unlabeled` types, so applying or removing the label triggers a fresh workflow run — removing the label re-fails the check exactly as expected. The label itself is auto-ensured by an idempotent `gh label create` step at job start (workflow gains `issues: write` permission) so a fresh fork or repo clone never hits a "label does not exist" failure mode. The override applies to the bundle-delta gate only; the absolute-threshold `lhci assert` step (`total-byte-weight`, `resource-summary:script:size`, `cumulative-layout-shift`) still hard-blocks unconditionally — the override is for "we expected this growth," not for "ignore the budget entirely."

- **Lighthouse CI restored as a PR gate.** `lighthouse-ci.yml` was deleted in v1.7.42 (#682) because the bubbletea/headless start failure made it red on every run, and the budget thresholds in `.lighthouserc.json` had not been re-baselined since the v1.7.30-era webui bundle. With `--no-tui` fixing the start failure, the gate is reinstated with re-calibrated thresholds: hard gates at `total-byte-weight ≤ 349580 B` (was 180000) and `resource-summary:script:size ≤ 329757 B` (was 120000), reflecting a webui bundle that has roughly doubled in transferred bytes and tripled in script transfer size since the original baseline. Soft warnings on `largest-contentful-paint ≤ 1300 ms` and `speed-index ≤ 1100 ms` (tightened from 1500) match the new p95+20% calibration; `first-contentful-paint`, `total-blocking-time`, and `cumulative-layout-shift` keep their previous thresholds since the calibration values were within or below the pre-v1.7.42 envelope. Trigger paths broadened to include `tests/lighthouse/**` and the workflow itself so calibration-script edits cycle through CI immediately. `tests/lighthouse/calibrate.sh` and `tests/lighthouse/budget-check.sh` updated to invoke `--no-tui`. Re-baseline locally with `./tests/lighthouse/calibrate.sh` whenever the bundle moves materially. The `weekly-regression.yml` Lighthouse + Playwright steps now also use `--no-tui` and drop the `env -i HOME=$HOME PATH=$PATH TERM=xterm-256color` workaround prefix that the v1.7.42 audit flagged as an unsuccessful mitigation.

- **`.lighthouserc.json` `startServerReadyPattern` corrected to `"Web server: http"`.** The previous value `"Listening on"` did not match anything `agent-deck web` actually prints (it logs `Web server: http://<addr>` at `cmd/agent-deck/main.go:708`), so `lhci collect`'s in-process server start would always time out before falling back to whatever was already bound on the listen address. Local invocations of `budget-check.sh` worked anyway because the script starts the server externally before invoking `lhci`, but the pattern mismatch would have surfaced as a 30-second startup stall on the first PR run of the reinstated workflow. The new value matches the actual log line emitted on every successful start.
