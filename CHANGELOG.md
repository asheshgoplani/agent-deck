...
### Fixed
- **Rogue telegram pollers no longer spawn when a conductor launches non-conductor claude children on the same host** ([#59](https://github.com/asheshgoplani/agent-deck/issues/59); poller-storm observed 2026-04-22, 6–8 duplicate `bun telegram` processes running concurrently against the conductor's bot token, producing a Bot API 409 Conflict storm and silently dropping inbound messages). v1.7.40 tried to solve this by stripping `TELEGRAM_STATE_DIR` from every non-conductor spawn (`internal/session/env.go:343 telegramStateDirStripExpr`) and wrapping the final claude exec in `env -u TELEGRAM_STATE_DIR ` (`internal/session/instance.go:557 execEnvPrefix`). Both strips do what they say, but they are not enough on their own: the Claude Code telegram plugin is enabled **globally** in the profile's `settings.json` per the v3 topology (see `internal/session/telegram_validator.go:66 GLOBAL_ANTIPATTERN`), so removing `TELEGRAM_STATE_DIR` just makes the plugin fall back to its **default** state dir `~/.claude/channels/telegram/` — which on a conductor host is the real bot-token directory. The worker then reads the conductor's `.env`, spawns a second `bun telegram` poller, and races the conductor for `getUpdates` on the same token.

  v1.7.68 adds a categorically different layer on top: every non-conductor claude worker now spawns under an **ephemeral scratch `CLAUDE_CONFIG_DIR`** that shallow-mirrors the ambient profile except for `settings.json`, which is rewritten with `enabledPlugins["telegram@claude-plugins-official"] = false`. The plugin never loads, so there is no opportunity for it to discover the default state dir in the first place. Mechanics:
  - `Instance.WorkerScratchConfigDir string` (serialized, `json:"worker_scratch_config_dir,omitempty"`) is the per-instance scratch path.
  - `Instance.NeedsWorkerScratchConfigDir()` reuses the existing `telegramStateDirStripExpr(inst) != ""` predicate — so the strip (S8) and the disable (this fix) apply to exactly the same set of sessions. Conductors (`Title` prefixed `conductor-`), explicit telegram channel owners (`Channels` contains `plugin:telegram@...`), and non-claude tools (codex, gemini, copilot, shell) are unaffected and continue to use the ambient profile.
  - `Instance.EnsureWorkerScratchConfigDir(source)` creates `~/.agent-deck/worker-scratch/<instance-id>/`, writes the mutated `settings.json`, and symlinks every other top-level entry from the source profile (commands, agents, plugins cache, `.claude.json` auth, etc.). Idempotent — safe to call on every `Start` / `StartWithMessage` / restart path.
  - `Instance.prepareWorkerScratchConfigDirForSpawn()` wires the Ensure call into all three spawn paths in `internal/session/instance.go` (fresh Start, StartWithMessage, and the restart fallback that recreates the tmux session).
  - `buildClaudeCommandWithMessage` and `buildBashExportPrefix` both prefer `Instance.WorkerScratchConfigDir` over `GetClaudeConfigDirForInstance(inst)` when it is set, so both the inline `CLAUDE_CONFIG_DIR=… claude` and the `export CLAUDE_CONFIG_DIR=…;` bash prefix forms route through the scratch dir.
  - `Instance.CleanupWorkerScratchConfigDir()` is called from `Instance.Kill` / `KillAndWait` so short-lived workers don't leak scratch dirs across reboots.

  6 new tests in `internal/session/worker_scratch_test.go`: `TestEnsureWorkerScratchConfigDir_DisablesTelegramPlugin` asserts the scratch settings.json pins telegram false while preserving other plugin flags and keeps the commands dir reachable via symlink; `…_ConductorKeepsAmbientProfile` and `…_ChannelOwnerKeepsAmbientProfile` assert the two legitimate-poller cases return empty string (use ambient profile); `…_NonClaudeToolSkipped` asserts codex/gemini are untouched; `…_TelegramAbsentStillPinsDisabled` asserts the plugin is pinned `false` even when the source profile has no `enabledPlugins.telegram` key (defense in depth against a concurrent conductor setup flipping it on behind the worker's back); `TestBuildClaudeCommand_UsesWorkerScratchConfigDir` is the wire check — with `Instance.WorkerScratchConfigDir` populated, the built command routes `CLAUDE_CONFIG_DIR` at the scratch dir, never at the ambient profile.

- **Orphan claude processes no longer survive `agent-deck session remove`** (same incident 2026-04-22: `PID 321456`, 33-hour orphan with `AGENTDECK_INSTANCE_ID` set and a since-deleted agent-deck record). Two distinct bugs fed into this:
  1. **`handleSessionRemove` only called `inst.Kill()` when the caller also passed `--prune-worktree`.** `agent-deck session remove --force` on a running session deleted the registry row and left the tmux scope + its claude child alive forever.
  2. **`Session.Kill` runs the SIGTERM→SIGKILL escalation in a background goroutine** (`internal/tmux/tmux.go:2051 ensureProcessesDead`). In a short-lived CLI process the goroutine is aborted when the CLI exits — so even code paths that did call `inst.Kill()` could race the process exit and leave SIGHUP-immune claude children alive.

  Fixes:
  - New `internal/tmux/ensure_pids_dead.go` exports `EnsurePIDsDead(pids []int, timeout time.Duration)`, a synchronous SIGTERM→SIGKILL primitive that blocks until every given PID is dead (or the timeout fires). Uses a deadline-aware sleep helper (`sleepUntilOrDuration`) and a 50ms poll loop after SIGKILL so it returns as soon as processes are reaped rather than sleeping blindly. PID-reuse is guarded by `isOurProcessLoose`, which extends the existing `isOurProcess` allowlist (claude/node/zsh/bash/sh/cat/npm) with `dash/ash/fish/bun/uv/python/sleep` so EnsurePIDsDead can reap any pane descendant callers hand it. Bounded by the short capture→kill window.
  - New `Session.KillAndWait() error` in the same file — tmux-session wrapper that does exactly what `Session.Kill` does, but calls `EnsurePIDsDead` inline rather than scheduling it on a goroutine.
  - New `Instance.KillAndWait() error` in `internal/session/instance.go` — parallels `Instance.Kill` (the async variant, which the TUI and web callers continue to use) and calls `Session.KillAndWait` under the hood. Factored through a private `killInternal(sync bool)` so future teardown cleanups (sandbox containers, keychain credentials, worker-scratch dirs) happen exactly once regardless of which variant fires.
  - `handleSessionRemove` (`cmd/agent-deck/session_remove_cmd.go`) now calls `inst.KillAndWait()` **unconditionally** before `storage.DeleteInstance`, not gated on `--prune-worktree`. The bulk `removeAllErrored` path was doing the same thing (Kill only inside `pruneSessionWorktree`) and received the same fix. `pruneSessionWorktree` itself was upgraded `Kill()` → `KillAndWait()` so every CLI-initiated remove path completes the process-tree teardown before the CLI returns.
  - Legacy `handleRemove` in `cmd/agent-deck/main.go` (the non-subcommand `agent-deck remove <id>` path) was similarly upgraded to `KillAndWait`.

  3 new tests cover the fix:
  - `TestEnsurePIDsDead_SynchronouslyKillsSigHupImmuneChild` (`internal/tmux/ensure_pids_dead_test.go`) spawns `sh -c 'trap "" HUP; sleep 30'` (emulating claude 2.1.27+), captures the PID, reaps the Go child via a background `cmd.Wait()` goroutine so `kill(pid, 0)` correctly returns ESRCH when the process dies (without the reaper, Go's unreaped-child zombie would make a signal-0 probe falsely report "alive"), calls `EnsurePIDsDead` with a 3s timeout, and asserts the PID is dead by the time the call returns. `TestEnsurePIDsDead_NoopOnEmptyPIDs` asserts the empty-slice input returns immediately (<250ms).
  - `TestSessionRemove_HandlerCallsKillUnconditionally` and `TestSessionRemove_AllErroredCallsKillUnconditionally` (`cmd/agent-deck/session_remove_kill_test.go`) are structural guards — they parse `session_remove_cmd.go`, extract the `handleSessionRemove` and `removeAllErrored` function bodies, and assert each calls `inst.Kill` / `inst.KillAndWait` in its mainline (not only inside a flag-gated helper). Cheap regression guard against the exact `--prune-worktree`-gate bug we just shipped the fix for.
