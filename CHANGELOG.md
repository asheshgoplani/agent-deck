...
- Shared attach: two people on one session (the maintainer through the remote deck at 200x60, a teammate from a terminal at 120x40) each see it full-size while they use it. The window now follows the client that most recently attached, typed or resized (`window-size=latest`; `largest` + `aggressive-resize` on a tmux without it) instead of being pinned to the smallest client, which boxed every larger terminal into a corner with dots (#2186's `smallest`) or to a synthetic size no client could show (the earlier `largest`). Because window-size is a per-window option that `resize-window` flips to `manual`, the policy is applied to every window at creation and again before every attach (TUI Enter, `session attach` on a remote, the web and embedded clients, Shift+Enter windows). An attach whose window still comes out smaller than the terminal logs the other clients and their sizes; nobody is ever detached (@siraben's #2186 kept for crossed dimensions: `latest` never produces a size no client has).
- Viewers indicator: who else has a session open. `👁️ N` on the session row and `viewers: ashesh (200x60, active 5s ago) · yasir (120x40, 3m ago)` in the preview, from `tmux list-clients` (agent-deck's own control pipes excluded; the tty owner names a viewer on tmux < 3.2); `session show --json` gains `viewers` and `list --json` carries it to remote decks (an older remote shows `viewers unknown (remote older than 1.16.11)`); `session viewers <session> [--json]` prints it; and the terminal that attaches gets a one-line `also viewing: yasir (120x40, …)` on its tmux status line.
