...
- Shared attach: two people on one session (the maintainer through the remote deck at 200x60, a teammate from a terminal at 120x40) each see it full-size while they use it. The window now follows the client that most recently attached, typed or resized (`window-size=latest`; `largest` + `aggressive-resize` on a tmux without it) instead of being pinned to the smallest client, which boxed every larger terminal into a corner with dots (#2186's `smallest`) or to a synthetic size no client could show (the earlier `largest`). Because window-size is a per-window option that `resize-window` flips to `manual`, the policy is applied to every window at creation and again before every attach (TUI Enter, `session attach` on a remote, the web and embedded clients, Shift+Enter windows). One window still has one size, so the terminal that is not being used shows the other person's size: clipped if it is smaller, the pane in the top-left corner with dots around it if it is larger, until you type or resize (that is tmux; no policy gives two differently sized terminals a full window at once, and nothing resizes anyone else's client). An attach whose window still comes out smaller than the terminal logs the other clients and their sizes; nobody is ever detached (@siraben's #2186 kept for crossed dimensions: `latest` never produces a size no client has). The web, embedded and Shift+Enter attaches honour `[tmux.options]` (`window-size = "smallest"` opts out) like every other path.
