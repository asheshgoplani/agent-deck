...
A conductor that launches workers on another host does not get their completions for free: transition notifications are parent-linked, and a `parent_session_id` cannot point across machines. `remote drain <name>` closes that gap by pulling: it reads the remote's records over the same SSH path (consuming nothing there) and writes them into the local inbox. Manual drains still include all exported records. Automatic heartbeat drains only configured remotes hosting sessions registered locally as children of that conductor, and passes those child IDs as a filter. This keeps another conductor's records and historical remote backlog out of its inbox. An unreachable remote is logged once until it recovers; only new inbox records wake the conductor.
