...
- Claude sessions: injected via the synchronous SessionStart hook (`additionalContext`), which re-fires on resume, `/clear`, and compaction — the primer survives a resume natively; every claude spawn installs/upgrades the hooks in its config dir, so CLI-only installs are covered
