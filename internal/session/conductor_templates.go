...
**Substate (Claude sessions only; refines status in ` + "`" + `list` + "`" + `/` + "`" + `show` + "`" + ` JSON):** ` + "`" + `auth-401` + "`" + ` covers two different pane banners. A credential banner (` + "`" + `Please run /login` + "`" + `, ` + "`" + `API Error: 401` + "`" + `) means the fleet is HOLDING the session; restarting will NOT fix it. Check ` + "`" + `session show --json <id>` + "`" + ` for the ` + "`" + `auth_hold` + "`" + ` object (the authoritative source, present even after the pane exits) and escalate for re-login. A dropped-socket banner (` + "`" + `socket connection closed` + "`" + `) also classifies as ` + "`" + `auth-401` + "`" + ` but is NOT held and IS restart-recoverable: restart it. ` + "`" + `model-unavailable` + "`" + ` means the selected model is down (shows as error, not running); self-heal currently only observes this and takes no action, so switch it yourself with ` + "`" + `agent-deck -p <PROFILE> session set <id> model <model>` + "`" + ` then ` + "`" + `agent-deck -p <PROFILE> session restart <id>` + "`" + `. ` + "`" + `idle-at-empty-prompt` + "`" + ` (shown as coarse status ` + "`" + `idle` + "`" + ` or ` + "`" + `waiting` + "`" + `) means the session is genuinely sitting at its prompt with nothing happening. ` + "`" + `awaiting-choice` + "`" + ` (also coarse ` + "`" + `idle` + "`" + `/` + "`" + `waiting` + "`" + `) means a permission dialog or a decision menu is on screen and a HUMAN must resolve it: ` + "`" + `session nudge` + "`" + ` refuses it with ` + "`" + `SESSION_AWAITING_CHOICE` + "`" + ` because sending would dismiss the prompt and paste its options into the composer as text. Approve a safe permission prompt deliberately (` + "`" + `session send <id> "1"` + "`" + ` for Claude, ` + "`" + `session approve <id> once` + "`" + ` for Codex); escalate a decision menu to the user and leave it untouched. Never restart-loop an ` + "`" + `error` + "`" + ` session that ` + "`" + `auth_hold` + "`" + ` confirms is credential-held.
