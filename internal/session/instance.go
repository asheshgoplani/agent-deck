	...
	if baseCommand == "claude" {
		...
		baseCmd = fmt.Sprintf(
			`session_id=$(%sclaude -p "." --output-format json 2>/dev/null | jq -r '.session_id' 2>/dev/null) || session_id=""; `+
				...
				`%sclaude --resume "$session_id"%s%s; `+
				`else %sclaude%s%s; fi`,
			configDirPrefix, configDirPrefix, addDirFlag, dangerousFlag, configDirPrefix, addDirFlag, dangerousFlag)
		...
		if message != "" {
			...
			baseCmd = fmt.Sprintf(
				`session_id=$(%sclaude -p "." --output-format json 2>/dev/null | jq -r '.session_id' 2>/dev/null) || session_id=""; `+
					...
					`%sclaude --resume "$session_id"%s%s; `+
					`else %sclaude%s%s; fi`,
				configDirPrefix, escapedMsg, configDirPrefix, addDirFlag, dangerousFlag, configDirPrefix, addDirFlag, dangerousFlag)
		}
