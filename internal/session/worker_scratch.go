...
// workerScratchDirFor returns the scratch dir for an instance. The name is
// derived from everything that determines the scratch's CONTENTS — the
// source profile plus the resolved plugin topology — and deliberately NOT
// from the instance ID.
//
// Keying by instance ID gave every session a unique CLAUDE_CONFIG_DIR
// path, which silently breaks auth on macOS: Claude Code keys its Keychain
// OAuth entry by the literal config-dir path, so a per-session path never
// has a credential entry and the session starts "Not logged in". Only
// visible once an explicit config_dir is set, since buildBashExportPrefix
// exports CLAUDE_CONFIG_DIR only in that case.
//
// Content-keying makes the path stable, so one login per (profile,
// topology) authenticates every session sharing it. It is also safe under
// concurrent seeding: equal keys imply equal generated bytes, so racing
// writers converge instead of clobbering each other. Different source
// profiles (different Claude accounts) and different plugin topologies
// still get separate dirs, preserving isolation.
func workerScratchDirFor(i *Instance, sourceProfileDir string) string {
	return filepath.Join(workerScratchDirRoot(), workerScratchKey(i, sourceProfileDir))
}

// workerScratchKey hashes the scratch's content-determining inputs into a
// short, filesystem-safe directory name. Lists are sorted and tagged so
// ordering churn and cross-category collisions ("deny:x" vs "allow:x")
// cannot produce the same key.
func workerScratchKey(i *Instance, sourceProfileDir string) string {
	h := sha256.New()
	writeField := func(tag, value string) {
		// NUL separator: no plugin id or path may contain it, so distinct
		// field sequences can never hash to the same byte stream.
		h.Write([]byte(tag))
		h.Write([]byte{0})
		h.Write([]byte(value))
		h.Write([]byte{0})
	}
	writeField("source", sourceProfileDir)
	for _, group := range []struct {
		tag string
		ids []string
	}{
		{"deny", computeDenyList(i)},
		{"allow", computeAllowList(i)},
		{"channel", computeChannelPluginAllowList(i)},
	} {
		ids := append([]string(nil), group.ids...)
		sort.Strings(ids)
		for _, id := range ids {
			writeField(group.tag, id)
		}
	}
	return fmt.Sprintf("profile-%x", h.Sum(nil)[:8])
}
...
func (i *Instance) EnsureWorkerScratchConfigDir(sourceProfileDir string) (string, error) {
	...
	scratch := workerScratchDirFor(i, sourceProfileDir)
...
// CleanupWorkerScratchConfigDir detaches this session from its scratch dir.
//
// It deliberately does NOT delete the directory. Since workerScratchDirFor
// became content-keyed, a scratch dir is shared by every session with the
// same (source profile, plugin topology) — removing it here would yank the
// CLAUDE_CONFIG_DIR out from under other running sessions, taking their
// auth and plugin set with it.
//
// Leaving it is safe and cheap: the contents are a pure function of the
// key, so the next spawn re-seeds identical bytes, and the directory count
// is now bounded by the number of distinct topologies rather than by the
