...
def send_to_conductor(
    ...
    # on success we re-fetch the clean reply via get_session_output (`session
    # output --json` -> content) rather than parsing the raw `--wait` pane
    # capture (issue #926). A timeout where the agent is still running (rather
    # than a genuine send failure) is surfaced as still_running so the caller
    # can await the reply asynchronously instead of reporting a false failure
    # (#1404).
    result = run_cli(
    ...
    if result.returncode != 0:
        ...
        # reply asynchronously rather than reporting a false failure (#1404).
        ...
        log.error("Failed to send to conductor: %s", stderr)
        return False, "", False
    # Re-fetch the clean reply via get_session_output rather than the raw
    # `--wait` pane capture (issue #926).
