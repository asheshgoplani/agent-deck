...
// Lower bands surface higher in the group's list. Archive is the strongest
// signal: a pinned-but-archived session loses its anchored slot and sinks to
// the archived band until unarchived (design point 4).
//
...
//	3  archived     always last, regardless of pin
...
// stablePinPartition reorders insts in place into pin-top, normal, pin-bottom,
// and archived bands (pinZone), preserving the relative order of sessions
// within each band. Unlike SortInstancesByActionable it never reorders by
// status/recency, so it is safe to run on every render (Flatten): it moves only
// pinned/archived rows, leaving the load-time actionable order — and any live
// K/J manual order — of the normal band untouched. This is what makes a pin
// edit take effect live instead of only after a restart.
...
// the pin zone (see pinZone); within the normal and archived zones the existing
// actionable tiers apply, and within the pin-top/pin-bottom bands sessions are
// ordered by Order alone (fully fixed — status and recency are ignored, so K/J
// reordering still works inside a band).
//
// Normal/archived zone key precedence:
//
...
func SortInstancesByActionable(insts []*Instance) {
	sort.SliceStable(insts, func(i, j int) bool {
		...
		// Normal (1) and archived (3) bands keep the actionable tiers.
