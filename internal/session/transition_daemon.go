...
func (d *TransitionDaemon) recordTerminalTurns(
	...
	for id, to := range statuses {
		...
		turnID := signal
		if turnID == "" {
			turnID = resultTurnID(inst)
		}
		if result, err := CaptureSessionResult(ResultIdentity{SessionID: id, TurnID: turnID}, inst.ProjectPath); err == nil {
			d.notifier.NotifyResult(TransitionNotificationEvent{
				ChildSessionID: id, ChildTitle: inst.Title, Profile: profile,
				ToStatus: "result", Timestamp: time.Now(), LastOutputHash: turnID,
				DoneStatus: result.Verdict, DoneSummary: result.Source,
			})
		}
