	...
	for _, mode := range []string{"cold", "cached", "rejected-cache"} {
		for _, source := range []string{"subagent", "user"} {
			t.Run(mode+"/"+source, func(t *testing.T) {
				...
				now := time.Now()
				hook := &HookStatus{Status: "running", SessionID: candidate, Event: "UserPromptSubmit", UpdatedAt: now}
				if mode == "cold" {
				...
				if source == "subagent" && inst.hookSessionID == candidate {
					t.Fatal("rejected child remains cached for the next status pass")
				}
			})
		}
	}
