...
func TestWriteGenericSessionBinding_NoClobberSiblings(t *testing.T) {
	...
	if err := db.WriteGenericSessionBinding("i1", "g-1", time.Unix(50, 0).UTC()); err != nil {
		t.Fatal(err)
	}
	...
	if err := db.WriteGenericSessionBinding("i1", "", time.Time{}); err != nil {
		t.Fatal(err)
	}
	m = readToolData(t, db, "i1")
...
func TestWriteGenericSessionBinding_ZeroDetectedAtStampsNow(t *testing.T) {
	...
	if err := db.WriteGenericSessionBinding("i1", "sid", time.Time{}); err != nil {
		t.Fatal(err)
	}
...
func TestWriteGenericSessionBinding_ConcurrentLastWins(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		go func() {
			defer wg.Done()
			if err := db.WriteGenericSessionBinding("i1", "A", time.Now()); err != nil {
				errCh <- err
			}
		}()
		go func() {
			defer wg.Done()
			if err := db.WriteGenericSessionBinding("i1", "B", time.Now()); err != nil {
				errCh <- err
			}
		}()
	}
...
func TestSchemaExtrasZone_NoMigrationRequired(t *testing.T) {
	...
	if err := db.WriteGenericSessionBinding("x", "ok", time.Now()); err != nil {
		t.Fatal(err)
	}
}
...
func TestWriteGenericSessionBinding_EmptyObjectAndJSONNull(t *testing.T) {
	...
	if err := db.WriteGenericSessionBinding("empty-obj", "from-empty", time.Unix(42, 0).UTC()); err != nil {
	...
	if err := db.WriteGenericSessionBinding("empty-obj", "nope", time.Now()); err == nil {
