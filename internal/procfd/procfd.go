...
// descriptors currently open in pid. Descriptors that close mid-scan or whose
// path cannot be resolved are skipped, not errors. Probing a dead PID or
// another user's process fails; current macOS reports EINVAL for both (not
// ESRCH/EPERM as one might expect).
