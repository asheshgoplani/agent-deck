	...
	case "enter":
		if h.cursor < len(h.flatItems) {
			...
			if item.Type == session.ItemTypeSession && item.Session != nil {
				...
				// remote confirms. Otherwise focus it in the embedded pane.
...
func (h *Home) attachSelectedLegacy() tea.Cmd {
	...
	case session.ItemTypeRemoteSession:
		if item.RemoteSession != nil {
			// Same as Enter on a stopped remote row upstream: restart first,
			// attach over SSH otherwise.
			if strings.EqualFold(item.RemoteSession.Status, string(session.StatusStopped)) {
				return h.restartRemoteSession(item.RemoteName, item.RemoteSession.ID, item.RemoteSession.Title)
			}
