...
| Fork session | `internal/ui/home.go:5979` (`f` key, quick) | POST `/api/sessions/{id}/fork` | `ForkSession` | `handlers_sessions_test.go`, WebUI action tests | Creates fork with resume command. Web creates a plain tool-native fork; TUI quick fork also applies `[fork]` defaults. Fork availability is carried on the wire via `MenuSession.canFork` (backend-detected, independent of tool name). |
| Fork with dialog | `internal/ui/home.go:5997` (`F`/`shift+f`) | POST `/api/sessions/{id}/fork` | `ForkSession` | `handlers_sessions_test.go` | Dialog allows custom title + group. Shift+F title/group/branch/worktree controls are TUI-only until Web gets a dedicated async fork workflow. |
...
| Archive / unarchive session | `internal/ui/home.go:7237` (`A` key) | POST `/api/sessions/{id}/archive` | `ArchiveSession` / `UnarchiveSession` | `handlers_sessions_test.go`, `web_mutator_test.go` (`StopsProcess`) | Toggle. POST archives (sets ArchivedAt **and stops the process — captures auto-name, then runs `Kill()` off the request goroutine, mirroring the TUI's deferred `stopArchivedSession`**); DELETE same path unarchives. Archived rows marked on wire (`MenuSession.archived`), hidden client-side by default. Note: the `parityStore` fake only models the `archived` wire flag, so process-lifecycle parity is pinned by `web_mutator_test.go` against a real `Instance`, not by the parity suite. |
