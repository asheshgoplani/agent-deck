...
and adds configured MCPs to the project's local MCP configuration. These are
attach-only floors: removing an entry from config does not delete an existing
project attachment, and user-owned paths are never overwritten.
...
home; it never calls `codex plugin add`, upgrades a marketplace, or modifies a
group's plugin installation.
...
4. A Codex session receiving the configured skill and MCP floor in
   `.agents/skills` and local MCP configuration.
...
- Given a configured skill or MCP, when a Codex session is created or
  restarted, then Agent Deck materializes it through the existing project
  attachment path without clobbering user-owned files.
