...
func handleWatcherCreate(profile string, args []string) {
	...
	// Anything only held in a flag here is lost: the engine never sees it, the
	// adapter's Setup fails at startup, and the watcher silently never runs.
	switch adapterType {
	case "github":
...
func writeTopicWatcherSource(dir, adapterType, topic string) (bool, error) {
	...
	case !errors.Is(err, os.ErrNotExist):
		return false, fmt.Errorf("stat watcher.toml: %w", err)
	}

	header := fmt.Sprintf("# Auto-generated by: agent-deck watcher create %s\n", adapterType)
	if err := writeWatcherSourceToml(dir, header, [][2]string{{"topic", topic}}); err != nil {
		return false, err
	}
	return true, nil
}
...
// at 0600, atomically (temp + rename). Settings are written in the given order
// so the file is deterministic, and as TOML strings so they decode into the
// engine's map[string]string Settings.
