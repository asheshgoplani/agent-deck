...
func handleAdd(profile string, args []string) {
	...
	if wtBranch != "" {
		...
		if backend == nil {
			fmt.Fprintf(os.Stderr, "Error: %s is not a VCS repository\n", path)
			os.Exit(1)
		}
...
func handleRemove(profile string, args []string) {
	...
	if inst.IsWorktree() {
		if backend, err := inst.Backend(); err != nil {
			if !*jsonOutput {
				fmt.Printf("Warning: failed to detect VCS backend: %v\n", err)
			}
		} else {
			if err := backend.RemoveWorktree(inst.WorktreeRepoRoot, inst.WorktreePath, false); err != nil {
				if !*jsonOutput {
					fmt.Printf("Warning: failed to remove worktree: %v\n", err)
				}
			}
			_ = backend.PruneWorktrees(inst.WorktreeRepoRoot)
		}
	}
