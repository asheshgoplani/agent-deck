...
- Start machine triage with `agent-deck status --json`, not `list --json`: the compact status payload avoids loading the full session inventory into context.
- Find conversation text with `agent-deck session search <query>`, not raw transcript grep: the indexed search returns bounded, ranked matches instead of whole transcript files.
- Supervise children with `agent-deck session children --follow --until-done`, not a polling loop: the blocking event stream removes repeated turns and repeated inventory reads.

`session output` and `--pane` builds that expose `--max-tokens` default to a bounded agent-boundary response: keep the cap for routine reads, and use the persisted full-output recovery path only when the omitted middle is actually needed. This is the token-cost guardrail from #2050. Conductor templates from #2051 apply the three cheap-path rules above and reference `HEARTBEAT_RULES.md` by resolved path instead of replaying its contents on every heartbeat.

## Recently Added Surfaces

- **Adopted agents, phase 1:** `agent-deck agents [--json] [--no-remote]` lists read-only definitions by machine; `agent-deck agent show <name> [--json]` inspects one; `agent-deck agent adopt <target>` emits a disabled plan and never changes the source. Adoption is a dry run unless `--write` is passed. Some newer builds also expose an agent-health view; check `agent-deck agent --help` before using it.
- **Session context inspector:** builds containing it expose `agent-deck session context <id|title>` to measure fixed startup/context overhead. Prefer it to reconstructing startup context from transcript files; confirm its flags with the installed binary.
- **Remote completion drain:** builds containing it expose `agent-deck remote drain` to pull completed remote turns into the local completion ledger. The v1.14.0 help surface does not yet advertise it, so feature-detect with `agent-deck remote --help`.
- **Remote host statistics:** newer remote dashboards surface bounded CPU/RAM/disk/load probes and a `remote stats` CLI. Configure the local status block with `[system_stats]`; feature-detect the remote command before automation.
- **Named account slots:** define `[profiles.<slot>.claude].config_dir` (and the corresponding Codex/DeepSeek slot where supported), then use `agent-deck add --account <slot>` or account-aware launch/fork flags shown by that build. `session switch-account` migrates an existing Claude conversation.
